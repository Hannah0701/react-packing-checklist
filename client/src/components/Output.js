...
export const Output = (props) => {
    ...
    useEffect(() => {
      ...
      newPackingList["Personal care"].push("Toiletries", "Personal Medications", "General Medications (e.g. painkillers, travel sickness)", "First Aid Kit", "Hairbrush/comb", "Deodorant", "Perfume/aftershave", "Moisturiser", "Toothbrush", "Toothpaste", "Shampoo", "Conditioner");
    ...
    const getAgeBasedItems = (age) => {
      ...
      ageBasedItems["Food"].push("Baby food");
      } else if (age <= 12) {
      ageBasedItems["Entertainment"].push("Toys", "Coloring books");
      ageBasedItems["Food"].push("Snacks for kids");
      } else if (age <= 18) {
        ageBasedItems["Electronics"].push("Handheld games console");
      } else {
        ageBasedItems["Personal care"].push("Makeup", "Shaving kit");
      }
  
      return ageBasedItems;
    };
                ...
                {Object.keys(packingList).map(category => (
                  <div className="output-category" key={category}>
                    ...
                    <div className="output-list-container">
                      {packingList[category].concat(getAgeBasedItems(holidayMaker.age)).map((item, index) => (
